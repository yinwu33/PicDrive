...
the trainer uses, and the per-step reward is read back split into the four terms it
was summed from -- vehicle collision, off-road, goal, and the classic-dynamics jerk
penalty (see the TEDDY_DBG_* block in drive.h).
...
Usage:
    .venv/bin/python -m pufferlib.teddy.drive.trace_agent --steps 400
    .venv/bin/python -m pufferlib.teddy.drive.trace_agent \\
        --policy experiments/puffer_teddy.pt --map-id 3 --steps 1280 \\
...
from pufferlib.teddy.drive.drive import DEBUG_FEATURE_NAMES, Drive
...
def draw_roads(ax, snap, label=False):
    ...
    from pufferlib.teddy.drive.viz import ROAD_STYLE
...
def video(rec, agent, snap, path, fps, title):
    ...
    from pufferlib.teddy.drive.viz import footprint
...
def scene_plot(rec, agent, snap, path, annotate_every, title, frame="traj"):
    ...
    *where* the reward was paid (map), *how much* each term was worth on each step
    ...
    from pufferlib.teddy.drive.viz import ROAD_STYLE, footprint
...
def main():
    ...
    p.add_argument("--env-name", default="puffer_teddy", help="config to take the env settings from")
