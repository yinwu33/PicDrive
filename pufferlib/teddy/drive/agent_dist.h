...
// and loaded once.
#ifndef TEDDY_AGENT_DIST_H
#define TEDDY_AGENT_DIST_H
...
static int agent_dist_load(const char *path) {
    ...
    if (f == NULL) {
        fprintf(stderr, "[teddy] cannot open agent distribution '%s'\n", path);
        return 0;
    }

    // "GIGADST1" is the on-disk format version stamped by
    // data_utils/womd/build_agent_dist.py, not a package name: `teddy` reads the same
    // resources/drive/agent_dist.bin that `giga` does, and renaming the magic here
    // would just stop it loading.
    char magic[8];
    if (fread(magic, 1, 8, f) != 8 || memcmp(magic, "GIGADST1", 8) != 0) {
        fprintf(stderr, "[teddy] '%s' is not a GIGADST1 file\n", path);
        fclose(f);
        return 0;
    }
    ...
    if (fread(&num_types, sizeof(int), 1, f) != 1 || num_types <= 0 || num_types > AGENT_DIST_MAX_TYPES) {
        fprintf(stderr, "[teddy] '%s' has a bad type count (%d)\n", path, num_types);
        fclose(f);
        return 0;
    }
    ...
    for (int i = 0; i < num_types; i++) {
        ...
        if (fread(&t->type, sizeof(int), 1, f) != 1 || fread(&t->prob, sizeof(float), 1, f) != 1 ||
            ...
            fprintf(stderr, "[teddy] '%s' truncated in the type header\n", path);
            fclose(f);
            return 0;
        }
    }
    ...
    for (int i = 0; i < num_types; i++) {
        ...
        if (fread(t->rows, sizeof(float), (size_t)t->num_rows * 3, f) != (size_t)t->num_rows * 3) {
            fprintf(stderr, "[teddy] '%s' truncated in the row payload\n", path);
            fclose(f);
            return 0;
        }
        acc += t->prob;
...
static void agent_dist_sample(TeddyRng *rng, int *out_type, float *out_length, float *out_width, float *out_height) {
    ...
    int row = teddy_rand_int(rng, 0, t->num_rows - 1);
    *out_type = t->type;
