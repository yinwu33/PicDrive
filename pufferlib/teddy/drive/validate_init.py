...
*joint* distribution of the WOMD corpus rather than just the marginals.
    ...
    .venv/bin/python -m pufferlib.teddy.drive.validate_init
"""
...
def main():
    ...
    # -- ego observation width ------------------------------------------------
    # giga appends a 13-value conditioning block here; teddy has none, so the ego
    # slice is ocean's plain state. Checking it is cheap and catches the one mistake
    # a half-finished conditioning removal makes: a policy silently reading partner
    # features as if they were ego state.
    print("\n-- ego observation --")
    obs = np.asarray(env.observations)
    ego_dim = 11 if env.dynamics_model == "jerk" else 8
    # In render_state mode -- what this script runs in -- the ego vector *is* the
    # whole observation: the scene reaches the policy as pixels, not entity sets.
    expected = ego_dim if env.obs_mode == "render_state" else (
        ego_dim + env.partner_features * env.max_partner_objects
        + env.road_features * env.max_road_objects
    )
    print(f"  obs_mode {env.obs_mode}   ego {ego_dim}   total {obs.shape[1]}   expected {expected}")
    if obs.shape[1] != expected:
        fails.append(f"observation width {obs.shape[1]} != expected {expected}")
