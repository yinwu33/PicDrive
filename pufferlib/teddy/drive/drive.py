...
class Drive(pufferlib.PufferEnv):
    def __init__(
        ...
        self.spawn_heading_jitter_deg = spawn_heading_jitter_deg
        self.wrong_way_frac = wrong_way_frac
        self.num_waypoints_max = num_waypoints_max
        ...
        agent_offsets, map_ids, num_envs = binding.shared(
            ...
            num_maps=num_maps,
            agents_per_map_min=self.agents_per_map_min,
        ...
        for i in range(num_envs):
            ...
            env_id = binding.env_init(
                ...
                map_dir=map_dir,
                agent_dist_path=self.agent_dist_path,
                agents_per_map_min=self.agents_per_map_min,
                agents_per_map_max=self.agents_per_map_max,
                spawn_speed_max=self.spawn_speed_max,
                spawn_heading_jitter_deg=self.spawn_heading_jitter_deg,
                wrong_way_frac=self.wrong_way_frac,
                num_waypoints_max=self.num_waypoints_max,
    ...
    def get_scene_snapshot(self, env_index):
        """Agents and roads of one packed scene, in the simulator's own frame.

        Used by viz.py to check the random initialization. Both halves come from the
        same C env on purpose: the sim subtracts the map centroid (set_means), so
        re-reading road geometry from the .bin would put the road in a different
        ...
        return dict(
            ...
            road_xy=road_xy[:n_pts],
            road_meta=road_meta[:n_polys],
        )
