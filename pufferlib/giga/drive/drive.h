...
#endif
// How many *other* agents reach the vector observation. Gigaflow keeps this well
...
static int giga_is_offroad(Drive *env, Entity *agent) {
...
static float giga_clearance(Drive *env, int agent_idx, float x, float y, int count) {
    ...
    for (int j = 0; j < count; j++) {
        ...
        float slack = sqrtf(dx * dx + dy * dy) - (ra + ro + GIGA_SPAWN_GAP);
        if (slack < best)
            best = slack;
    }
    return best;
...
static void giga_sample_route(Drive *env, int agent_idx) {
    Entity *a = &env->entities[agent_idx];
    ...
    if (lg->num_lanes == 0) {
        ...
        int n_wp = giga_rand_int(&env->rng, 0, env->num_waypoints_max) + 1; // + final goal
        if (n_wp > MAX_WAYPOINTS)
            n_wp = MAX_WAYPOINTS;
        int lane = a->spawn_lane;
        float s = a->spawn_s;
        for (int i = 0; i < n_wp; i++) {
            float want = giga_rand_range(&env->rng, env->waypoint_min_dist, env->waypoint_max_dist);
            int next_lane;
            float next_s;
...
static void giga_place_agent(Drive *env, int agent_idx, int count) {
    Entity *a = &env->entities[agent_idx];
    LaneGraph *lg = &env->lane_graph;
    ...
    for (int attempt = 0; attempt < GIGA_SPAWN_ATTEMPTS; attempt++) {
        int lane;
        float s;
        ...
        h += giga_rand_normal(&env->rng) * jitter;
        if (giga_rand_float(&env->rng) < env->wrong_way_frac)
            h += (float)M_PI;

        float clear = giga_clearance(env, agent_idx, x, y, count);
        ...
        // giga_sample_route then has to fall back to a goal it cannot reach by
    ...
    float speed = giga_rand_range(&env->rng, 0.0f, env->spawn_speed_max);
    ...
    giga_sample_route(env, agent_idx);
}
...
static void giga_build_agents(Drive *env, int n_agents) {
    if (n_agents < 1)
        n_agents = 1;
...
static void giga_set_active_agents(Drive *env) {
...
static void giga_spawn_all(Drive *env) {
    for (int i = 0; i < env->num_objects; i++)
...
void init(Drive *env) {
    ...
    // discarded, so giga and ocean put the same map in the same place.
    set_means(env);
    giga_build_agents(env, env->num_agents);
    // After giga_build_agents: the graph stores entity indices, and rebuilding the
...
// Scene snapshot for the visualization acceptance check (pufferlib/giga/drive/viz.py).
//
...
#define GIGA_SNAPSHOT_AGENT_FEATURES (11 + 2 * MAX_WAYPOINTS)

// out = {num_agents, num_road_polylines, num_road_points}
void c_giga_scene_sizes(Drive *env, int *out) {
    int polys = 0;
    int pts = 0;
    for (int i = env->num_objects; i < env->num_entities; i++) {
        polys++;
...
// agents:    float[num_agents][GIGA_SNAPSHOT_AGENT_FEATURES]
...
void c_giga_scene_dump(Drive *env, float *agents, float *road_xy, int *road_meta) {
    for (int i = 0; i < env->active_agent_count; i++) {
        Entity *a = &env->entities[env->active_agent_indices[i]];
        float *row = &agents[(size_t)i * GIGA_SNAPSHOT_AGENT_FEATURES];
        row[0] = a->x;
        row[1] = a->y;
        row[2] = a->heading;
        row[3] = a->length;
...
void compute_observations(Drive *env) {
    ...
    for (int i = 0; i < env->active_agent_count; i++) {
        ...
        for (int j = 0; j < env->num_actors; j++) {
            if (cars_seen >= MAX_PARTNER_OBS)
                break; // observation buffer only has room for MAX_PARTNER_OBS partners
            int index = -1;
            if (j < env->active_agent_count) {
                index = env->active_agent_indices[j];
            } else if (j < env->num_actors && env->static_agent_count > 0) {
...
void c_reset(Drive *env) {
    ...
    for (int x = 0; x < env->active_agent_count; x++) {
        ...
        // giga_spawn_all -> giga_sample_route already counted this episode's first
