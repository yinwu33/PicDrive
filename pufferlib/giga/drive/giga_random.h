...
#ifndef GIGA_RANDOM_H
#define GIGA_RANDOM_H
...
static inline uint32_t giga_rand_u32(GigaRng *rng) {
...
static inline void giga_rand_seed(GigaRng *rng, uint64_t seed, uint64_t stream) {
    rng->state = 0u;
...
// Uniform in [0, 1).
static inline float giga_rand_float(GigaRng *rng) {
    // 24 bits is the full mantissa of a float; taking more would just round.
    return (float)(giga_rand_u32(rng) >> 8) * (1.0f / 16777216.0f);
}

// Uniform in [lo, hi).
static inline float giga_rand_range(GigaRng *rng, float lo, float hi) {
    return lo + (hi - lo) * giga_rand_float(rng);
}
...
static inline int giga_rand_int(GigaRng *rng, int lo, int hi) {
    if (hi <= lo)
        return lo;
...
static inline float giga_rand_normal(GigaRng *rng) {
    float u1 = giga_rand_float(rng);
    float u2 = giga_rand_float(rng);
    if (u1 < 1e-7f)
        u1 = 1e-7f;
...
// would be.
static inline float giga_rand_xmix(GigaRng *rng, float a) {
    if (a <= 1.0f)
        return 1.0f;
    if (giga_rand_u32(rng) & 1u)
        return giga_rand_range(rng, 1.0f / a, 1.0f);
    return giga_rand_range(rng, 1.0f, a);
}
