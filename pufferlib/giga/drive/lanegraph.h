...
#ifndef GIGA_LANEGRAPH_H
#define GIGA_LANEGRAPH_H
...
static int sample_lane_point(const LaneGraph *lg, GigaRng *rng, int *out_lane, float *out_s) {
    if (lg->num_lanes == 0 || lg->total_length <= 0.0f)
        return 0;

    float target = giga_rand_float(rng) * lg->total_length;
...
static float lane_walk_forward(const LaneGraph *lg, GigaRng *rng, int lane_idx, float s, float target_dist,
    ...
    while (travelled < target_dist && guard++ < 64) {
        ...
        lane_idx = lane->succ[giga_rand_int(rng, 0, lane->num_succ - 1)];
        s = 0.0f;
    }
    *out_lane = lane_idx;
    *out_s = s;
