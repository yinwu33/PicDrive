...
static PyObject *my_shared(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    GigaRng rng;
    giga_rand_seed(&rng, (uint64_t)seed, 0x9E3779B97F4A7C15ULL);
    ...
    while (total_agent_count < num_agents && env_count < max_envs) {
        int map_id = giga_rand_int(&rng, 0, num_maps - 1);
        int n = giga_rand_int(&rng, agents_per_map_min, agents_per_map_max);
...
static int my_init(Env *env, PyObject *args, PyObject *kwargs) {
    ...
    env_config_set_giga_defaults(&conf);
...
#undef OVERRIDE_FLOAT
    ...
    giga_rand_seed(&env->rng, (uint64_t)env_seed, (uint64_t)(map_id * 2654435761u + 1u));
