...
static PyObject *giga_scene_sizes(PyObject *self, PyObject *args) {
    Env *env = unpack_env(args);
    if (!env)
        return NULL;
    int sizes[3];
    c_giga_scene_sizes((Drive *)env, sizes);
    return Py_BuildValue("(iii)", sizes[0], sizes[1], sizes[2]);
}

static PyObject *giga_scene_dump(PyObject *self, PyObject *args) {
    if (PyTuple_Size(args) != 4) {
        PyErr_SetString(PyExc_TypeError, "giga_scene_dump requires 4 arguments");
        return NULL;
    }
    Env *env = unpack_env(args);
    if (!env)
    ...
    if (!PyArray_Check(agents) || !PyArray_Check(road_xy) || !PyArray_Check(road_meta)) {
        PyErr_SetString(PyExc_TypeError, "giga_scene_dump outputs must be NumPy arrays");
        return NULL;
    }
    c_giga_scene_dump((Drive *)env, (float *)PyArray_DATA((PyArrayObject *)agents),
...
static PyMethodDef methods[] = {
    ...
    {"giga_scene_sizes", giga_scene_sizes, METH_VARARGS, "Snapshot buffer sizes: (agents, road polylines, road points)"},
    {"giga_scene_dump", giga_scene_dump, METH_VARARGS, "Dump agents and roads of one env in the map-centered frame"},
