...
already encodes every normalizer and clip that keeps the reconstruction inside
the JERK integrator's reachable set, and it is the exact code the Waymo branch
runs.  Two arguments differ from the Waymo call:
...
* ``goal`` -- the episode's own final box centre, held fixed.  With
  ``goal_behavior = 0`` the frozen head trained on a fixed endpoint that decays
  toward the vehicle, not a receding lookahead.
"""
